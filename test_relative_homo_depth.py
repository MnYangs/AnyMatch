"""
Evaluation script for relative homography estimation on RGB-depth image pairs.

This script evaluates feature matchers on DIODE depth datasets by:
1. Loading RGB-depth image pairs with homography ground truth
...
def load_vis_depth_pairs_npz(data_root, csv_path):
    """Load RGB-depth image pairs with synthetic homographies from a CSV index.

    Each row in the CSV contains a relative path to an RGB image.
    The corresponding depth path is derived by replacing '/outdoor/' with '/outdoor_depth/'.
...
def compute_mask(real_H, mkpts0, mkpts1, threshold=3):
    """Compute a binary mask indicating correct matches (symmetric epipolar error < threshold).
...
def eval_relapose(matcher, scene_pairs, save_figs, figures_dir=None,
    ...
    Returns:
        ...
        precs: Per-scene precision results (empty placeholder).
        precs_no_inlier: Per-scene no-inlier precision (empty placeholder).
        agg_precs: Aggregated precision (empty placeholder).
        agg_precs_no_inlier: Aggregated no-inlier precision (empty placeholder).
    """
    scene_pose_auc = {}
    precs = {}
    precs_no_inlier = {}
...
def test_relative_pose_visdepth(method="xoftr", exp_name="VisDepth",
                                ransac_thres=1.5, print_out=False,
                                save_dir=None, save_figs=False, debug=False, args=None):
    """Run the full evaluation pipeline for RGB-depth homography estimation.
    ...
    # Data paths
    data_root = "./data/RealDataset/RGB-Depth/"
    csv_path = "./data/RealDataset/RGB-Depth/val_outdoor.csv"

    # Load pairs and matcher
    scene_pairs = load_vis_depth_pairs_npz(data_root, csv_path)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Benchmark Relative Pose on RGB-Depth')
