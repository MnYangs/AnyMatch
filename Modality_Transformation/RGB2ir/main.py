...
def all_gather(data):
    ...
    if not isinstance(data, torch.Tensor):
        ...
        tensor = torch.ByteTensor(storage).to("cuda:1")
    else:
    ...
    local_size = torch.LongTensor([tensor.numel()]).to("cuda:1")
    size_list = [torch.LongTensor([0]).to("cuda:1") for _ in range(world_size)]
    ...
    tensor_list = []
    for _ in size_list:
        tensor_list.append(torch.FloatTensor(size=(max_size,)).cuda(2).to(tensor_type))
    if local_size != max_size:
        padding = torch.FloatTensor(size=(max_size - local_size,)).cuda(2).to(tensor_type)
