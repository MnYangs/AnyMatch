...
class ImageEditor(nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad(), autocast("cuda:1"), self.model.ema_scope():
            cond = {
...
def compute_metrics(config,
                    ...
                    res = 512, 
                    seed = 0):
    editor = ImageEditor(config, model_path, vae_ckpt).cuda(2)
    clip_similarity = ClipSimilarity().cuda(2)
    ...
    for scale_txt in scales_txt:
        for scale_img in scales_img:
            ...
            while count < num_samples:
                ...
                gen = editor(sample["image_0"].cuda(2), sample["edit"], scale_txt=scale_txt, scale_img=scale_img, steps=steps)

                sim_0, sim_1, sim_direction, sim_image = clip_similarity(
                    sample["image_0"][None].cuda(2), gen[None].cuda(2), [sample["input_prompt"]], [sample["output_prompt"]]
                )
