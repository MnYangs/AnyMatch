"""
Evaluation script for relative homography estimation on RGB-event image pairs.

This script evaluates feature matchers on event camera datasets (e.g., DSEC) by:
1. Loading RGB-event image pairs with homography ground truth
...
def load_vis_event_pairs_npz(data_root, txt_file):
    """Load RGB-event image pairs with synthetic homographies from a text index.

    Each line in the text file contains a relative path to an RGB image.
    The corresponding event path is derived by replacing 'images' with 'events'.

    Args:
        data_root: Root directory of the dataset.
        txt_file: Path to the text index file listing image paths.

    Returns:
        scene_pairs: Dictionary with 'event' as key and list of pair dicts as value.
    ...
    with open(txt_file, 'r') as f:
        for line in f:
            line = line.strip()
            vis_path = os.path.join(data_root, line)
            event_path = os.path.join(data_root, line.replace('images', 'events'))

            if os.path.exists(vis_path) and os.path.exists(event_path):
                H0 = sample_homography(image_shape, vis_path, config).squeeze().numpy()
                H1 = sample_homography(image_shape, event_path, config).squeeze().numpy()

                pairs.append({'im0': vis_path, 'im1': event_path, 'H': H0})
                pairs.append({'im0': event_path, 'im1': vis_path, 'H': H1})
...
def eval_relapose(matcher, scene_pairs, save_figs, figures_dir=None,
    ...
    for scene_name, groups in scene_pairs.items():
        ...
        for i, pair in tqdm(enumerate(groups), smoothing=.1, total=len(groups)):
            ...
            # Read, resize, and apply homography warp
            if read_color:
                ...
                im0_t = torch.tensor(im0, dtype=torch.float32).permute(2, 0, 1).unsqueeze(0) / 255.0
                H_t = torch.tensor(real_H, dtype=torch.float32).unsqueeze(0)
                im0_warped = H_transform(im0_t, H_t).squeeze().permute(1, 2, 0).cpu().numpy() * 255
                ...
                im0 = cv2.resize(im0, (640, 480))
                im1 = cv2.resize(im1, (640, 480))
                im0_t = torch.tensor(im0, dtype=torch.float32).unsqueeze(0).unsqueeze(0) / 255.0
                H_t = torch.tensor(real_H, dtype=torch.float32).unsqueeze(0)
                im0_warped = H_transform(im0_t, H_t).squeeze().cpu().numpy() * 255
                real_H = H_t.squeeze().cpu().numpy()

            # Run feature matching
            match_res = matcher(im0_warped, im1)
            ...
            if len(mkpts0) >= 4:
                ret_H, _ = cv2.findHomography(mkpts1, mkpts0, cv2.RANSAC)
                mean_dist = compute_mean_distance(real_H, ret_H, H_img, W_img,
                                                  ...
                                                  image2=img0, method=method) if ret_H is not None else np.inf
            else:
                ret_H = None
                mean_dist = np.inf
...
def test_relative_pose_visevent(method="xoftr", exp_name="VisEvent",
                                ransac_thres=1.5, print_out=False,
                                save_dir=None, save_figs=False, debug=False, args=None):
    """Run the full evaluation pipeline for RGB-event homography estimation.
    ...
    # Data paths
    data_root = './data/RealDataset/RGB-EVENT/DSEC/'
    txt_file = "./data/RealDataset/RGB-EVENT/DSEC/event_list.txt"

    # Load pairs and matcher
    scene_pairs = load_vis_event_pairs_npz(data_root, txt_file)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Benchmark Relative Pose on RGB-Event')
